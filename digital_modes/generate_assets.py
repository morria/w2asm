...
# nfft = FFT size (larger = more freq resolution for narrow modes)
MODES = {
    'morse':         {'label': 'CW (Morse)',      'fc': 700,  'bw': 600,  'nfft': 512},
    'psk31':         {'label': 'PSK31',            'fc': 1000, 'bw': 400,  'nfft': 512},
    'psk63':         {'label': 'PSK63',            'fc': 1000, 'bw': 500,  'nfft': 512},
    'qpsk31':        {'label': 'QPSK31',           'fc': 1000, 'bw': 400,  'nfft': 512},
    'rtty45_170':    {'label': 'RTTY 45/170',      'fc': 1500, 'bw': 600,  'nfft': 512},
    'rtty50_170':    {'label': 'RTTY 50/170',      'fc': 1500, 'bw': 600,  'nfft': 512},
    'rtty100_850':   {'label': 'RTTY 100/850',     'fc': 1500, 'bw': 1600, 'nfft': 256},
    ...
    'dominoex11':    {'label': 'DominoEX 11',      'fc': 1500, 'bw': 600,  'nfft': 512},
    'mt63_1000':     {'label': 'MT63/1000',        'fc': 1500, 'bw': 1600, 'nfft': 256},
    'navtex':        {'label': 'NAVTEX',           'fc': 1000, 'bw': 800,  'nfft': 256},
    'usb':           {'label': 'USB (Voice)',       'fc': 1500, 'bw': 3000, 'nfft': 256},
    'lsb':           {'label': 'LSB (Voice)',       'fc': 1500, 'bw': 3000, 'nfft': 256},
    'am':            {'label': 'AM',               'fc': 1500, 'bw': 3000, 'nfft': 256},
...
def estimate_freq_offset(iq_block):
    """Estimate the frequency offset of an IQ block using spectral peak."""
    fft = np.fft.fft(iq_block)
    power = np.abs(fft) ** 2
    freqs = np.fft.fftfreq(len(iq_block), 1.0 / FS)

    # Smooth power spectrum to avoid noise peaks
...
def align_and_concat(data, indices):
    """Frequency-align IQ samples before concatenation.
    ...
    for idx in indices:
        ...
        offset = estimate_freq_offset(block)
        ...
        if abs(shift) > 1.0:  # only correct if meaningful
            t = np.arange(len(block)) / FS
            block = block * np.exp(1j * 2 * np.pi * shift * t)

        blocks.append(block)

    return np.concatenate(blocks), ref_freq
...
def generate_snr_comparison(mode_key, all_idx, all_modes, all_snrs, data, out_path):
    ...
    for snr in snr_levels:
        ...
        iq, ref_freq = align_and_concat(data, indices)
        freqs, times, spec = complex_spectrogram(iq, nfft=nfft)
        spec_db = 10 * np.log10(spec + 1e-12)
